...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const backendUrl = `${apiBaseUrl}/register`;
      ...
      <div className="w-full max-w-md p-8 rounded-3xl bg-neutral-900 border border-neutral-800 shadow-2xl">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent text-center">
            Join Life Optimizer Chat
          </h1>
          <p className="text-neutral-400 mt-2 text-sm text-center">
            Create an account to personalize your AI growth assistant.
          </p>
        </div>
        ...
        <p className="mt-6 text-center text-xs text-neutral-500">
          Already have an account? <Link href="/login" className="text-purple-400 hover:text-purple-300 font-medium ml-1 transition-colors">Log in here</Link>
        </p>
      </div>
    </div>
  );
